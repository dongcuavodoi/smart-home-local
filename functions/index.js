...
const devices = Object.entries(config).flatMap(([deviceId, deviceConf]) => {
    ...
    const controlProtocol = controlKinds.includes(deviceConf.control_protocol) ?
    ...
    return {
        ...
        payload: {
            commands: body.inputs[0].payload.commands.map((command) => {
                functions.logger.error(`Cloud fallback for ${command.execution[0].command}.`, `EXECUTE received for device ids: ${command.devices.map((device) => device.id)}.`);
                return {
...
exports.authorize = functions.https.onRequest((req, res) => {
    ...
    res.status(200).send(`<a href="${decodeURIComponent(redirectUri)}?code=placeholder-auth-code&state=${req.query.state}">Complete Account Linking</a>`);
});
